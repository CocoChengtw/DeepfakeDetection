"""
Extracts fixed number of frames from each video in a dataset using multithreading.
Input: metadata.csv with [name, split]
Output: PNG frames saved by split and video name
"""
...
def extract_and_save_frames(task_n, sp_meta):
    ...
    for i, r in sp_meta.iterrows():
        ...
        try:
            ...
            out_dir = (
                out_root + "/" + split + "/" + name.replace(".mp4", "")
            )  # TODO: watch out file name not end with .mp4
...
def process_with_threads(task_n, thr_cnt):
...
if __name__ == "__main__":
    ...
    for tk_name in task_name_l:
        ...
        # ------------
        process_with_threads(tk_name, 4)
        # ------------
